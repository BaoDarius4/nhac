...
module.exports = {
    ...
    usage: `tìm kiếm <link/query>`,
    run: async (client, message, args, cmduser, text, prefix) => {
      const { channel } = message.member.voice;
      if (!channel)  return message.channel.send(`:x: **bạn phải trong voice mới sài được lệnh này.**`);
      ...
      if (!args[0]) {
        let string = `${prefix}chơi <link/query>`
        let embed = new MessageEmbed()
        .setTitle("**:x: dell biết**")
        .setDescription(string)
        .setColor("#ff0000")
        if(message.guild.me.hasPermission("EMBED_LINKS")){
          message.channel.send(embed)
        }else{
          message.channel.send("**:x: dell biết**\n"+string)
        }
        return;
      }
      ///get the player
      ...
      playermanager(client, message, args, `Tìm kiếm:youtube`);
  }
};
/**
  * @INFO
